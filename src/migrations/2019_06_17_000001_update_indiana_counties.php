...
return new class extends Migration {
    ...
    public function up()
    {
        $indiana_area_id = DB::table('governmental_unit_areas')->where([
            'name' => 'Indiana'
        ])->first()->id;
        ...
        foreach (parse_ini_file(self::STATE_FILE) as $county_name => $county_geo) {
            ...
            $tax_id = DB::table('taxes')->where([
                'class' => self::CLASSES[$county_name],
            ])->first()->id;

            DB::table('tax_areas')->where([
                'tax_id' => $tax_id,
            ])->delete();

            DB::table('tax_areas')->insert([
                'tax_id' => $tax_id,
                'work_governmental_unit_area_id' => $indiana_area_id,
    ...
    public function down()
    {
        foreach (parse_ini_file(self::STATE_FILE) as $county_name => $county_geo) {
            ...
            $area_id = DB::table('governmental_unit_areas')->where([
                'name' => $county_name . ', IN',
            ])->first()->id;
