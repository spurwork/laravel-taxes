...
class AddDelawareIncomeTaxInformation extends Migration
{
    public function up(): void
    {
        Schema::create('delaware_income_tax_information', static function (Blueprint $table) {
            $table->increments('id');
        ...
        $income_tax_id = DB::table('taxes')->insertGetId([
            ...
            'class' => DelawareIncome::class,
        ]);

        $unemployment_tax_id = DB::table('taxes')->insertGetId([
            'name' => 'Delaware Unemployment Tax',
            'class' => DelawareUnemployment::class,
        ]);

        $id = DB::table('governmental_unit_areas')->where('name', 'Delaware')->first()->id;
    ...
    {
        $tax_id = DB::table('taxes')->where('class', DelawareIncome::class)->id;
        $unemployment_id = DB::table('taxes')->where('class', DelawareUnemployment::class)->id;
        ...
        Schema::drop('delaware_income_tax_information');
    }
}
